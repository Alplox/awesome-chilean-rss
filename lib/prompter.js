...
export async function promptStatus(feedName, feedUrl) {
  ...
  return withReadline(
    `     ⚠️  Estado para "${feedName}" (${feedUrl}): [a]ctive, [o]ffline, [b]roken, [n]o_feed, [e]mpty, s[t]ale (enter = no_feed): `,
    answer => {
      const map = { a: 'active', o: 'offline', b: 'broken', n: 'no_feed', e: 'feed_empty', t: 'stale' };
