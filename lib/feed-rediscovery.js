...
async function testFeeds(urls) {
  for (const url of urls) {
    ...
    if (result?.type && result.itemCount > 0) return { feedUrl: url, feedType: result.type, itemCount: result.itemCount };
  }
  return null;
}

export async function rediscoverFeed(siteUrl, preferredPatterns = []) {
