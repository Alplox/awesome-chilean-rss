...
function validateXmlItem(item, requireValidLink) {
  const { title, link } = extractItemFields(item);
  const valid = title.length >= 5 && (!requireValidLink || link.startsWith('http') || link.startsWith('/'));
  return { valid, title, link };
}

/**
 * Extrae campos identificadores de un item XML (RSS/Atom/RDF):
 * title, link y guid (Atom usa <id>, RSS usa <guid>).
 * @param {object} item - Item del feed
 * @returns {{ title: string, link: string, guid: string|null }}
 */
function extractItemFields(item) {
  ...
  if (typeof rawGuid === 'string') guid = rawGuid.trim();
  else if (rawGuid?.['#text']) guid = String(rawGuid['#text']).trim();
...
/**
 * Convierte una lista de items (XML o JSON) en su forma normalizada
 * { title, link, guid } para comparación de contenido.
 * @param {Array} itemList - Items crudos del feed
 * @param {'XML'|'JSON'} format - Formato de origen
 * @returns {Array<{ title: string, link: string, guid: string|null }>}
 */
function mapItemsToIdentifiers(itemList, format) {
  if (!Array.isArray(itemList)) return [];
  if (format === 'JSON') {
    return itemList.map(item => ({
      title: item?.title ? String(item.title).trim() : '',
      link: item?.url ? String(item.url).trim() : '',
      guid: item?.id ? String(item.id).trim() : null,
    }));
  }
  return itemList.map(item => extractItemFields(item));
}
...
export async function checkFeedUrl(url, options = {}) {
  ...
  const inspected = inspectFeedText(text, options);
  return inspected.error
    ? { ...inspected, code: null, ...redirectInfo }
    : { ...inspected, ...redirectInfo };
}

/**
 * Analiza el texto crudo de un feed (RSS/Atom/JSON/RDF) sin red.
 * Compartido por checkFeedUrl y el fallback de navegador headless.
 * @param {string} text - Contenido del feed
 * @param {object} options - Opciones (includeItems, requireValidLink)
 * @returns {{ type: string, itemCount: number, ... } | { error: string }}
 */
export function inspectFeedText(text, options = {}) {
  const {
  ...
  if (text.trimStart().startsWith('<html') || text.trimStart().startsWith('<!DOCTYPE')) {
    return { error: 'HTML (no es feed)' };
  }

  const type = detectFeedType(text);
  if (!type) return { error: 'no es RSS/Atom' };

  if (type === 'JSON') {
    try {
      return validateJsonFeed(text, '', includeItems);
    } catch {
  ...
  try {
    ...
    if (!feedData) return { error: 'sin canal' };

    if (feedData.itemCount === 0) {
      return { type, itemCount: 0, title: feedData.title, selfLink };
    }

    const itemList = feedData.itemList;
    const hasValidItem = itemList.some(item => validateXmlItem(item, requireValidLink).valid);
    if (!hasValidItem) return { error: 'items sin contenido válido' };

    const base = { type, itemCount: feedData.itemCount, title: feedData.title, lastItemDate: feedData.lastItemDate, selfLink };
    if (includeItems) base.items = mapItemsToIdentifiers(itemList, 'XML');
    return base;
  } catch (err) {
    console.warn(`[inspectFeedText] Parse error: ${err?.message ?? err}`);
 ...
 */
function validateJsonFeed(text, _url, includeItems = false) {
  ...
  const base = { type: 'JSON', itemCount, title: parsed.title || '(sin título)', lastItemDate };
  if (includeItems) base.items = mapItemsToIdentifiers(items, 'JSON');
  return base;
}
