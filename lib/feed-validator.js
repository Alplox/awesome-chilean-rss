...
export async function fetchSafe(url, options = {}) {
  ...
  for (let attempt = 0; attempt < FETCH_RETRIES; attempt++) {
    ...
    try {
      ...
      if ((res.status === 429 || res.status === 503) && attempt < FETCH_RETRIES - 1) {
        shouldRetry = true;
 ...
 */
export function parseFeedXml(parsed, _type) {
...
export async function checkFeedUrl(url, options = {}) {
  ...
  try {
    ...
    if (!feedData) return { error: 'sin canal', code: null };

    if (feedData.itemCount === 0) {
      return { type, itemCount: 0, title: feedData.title };
    }
    ...
    return { type, itemCount: feedData.itemCount, title: feedData.title, lastItemDate: feedData.lastItemDate };
