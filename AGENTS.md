...
| `node scripts/utils/find-duplicates.js`                                       | `scripts/utils/find-duplicates.js`         | Detect duplicates intra-DB (site URLs, rss_urls, root domains, IDs) + cross-file DB↔watchlist (site IDs/URLs, feed IDs/rss_urls) — catches incomplete promotions (exits 1 if found) |
| `node scripts/utils/find-duplicates.js --verbose`                             | same                                       | Same with clickable file:line links for each result (DB + WL)                                       |
| `node scripts/utils/find-duplicates.js --fix --dry-run`                       | same                                       | Preview prune of 41 cross-duplicate sites from watchlist (keep DB)                                  |
| `node scripts/utils/find-duplicates.js --fix --yes`                           | same                                       | Prune cross-duplicates from watchlist (keep DB); use `--keep watchlist` for opposite              |
| `node scripts/utils/find-duplicates.js --all --out reporte.txt`               | same                                       | Full report without truncation to file; `--json dupes.json` for machine-readable                   |
...
| `npm run check:overlap -- --update --validate`                               | same                                       | Also update feed status (active/stale/feed_empty/broken) reusing the same fetch results — saves a full validation pass                                                                                                                   |
...
| `node scripts/utils/standardize-feed-keys.js`                                | `scripts/utils/standardize-feed-keys.js`   | Reorder feed/site keys to canonical order (FEED_KEY_ORDER/SITE_KEY_ORDER) — handles `feeds-database.json` and `watchlist.json` (con `reason`); flags: `--file database\|watchlist\|all`, `--dry-run`, `--check` |
| `node scripts/utils/standardize-feed-keys.js --file watchlist --dry-run`     | same                                       | Preview only for watchlist (incluye `reason`)                                                                                                                                                                                            |
| `node scripts/utils/standardize-feed-keys.js --check`                        | same                                       | CI check — exit 1 si hay claves desordenadas                                                                                                                                                                                             |
...
scripts/
  ...
  utils/
    ...
    find-duplicates.js   — detect duplicates intra-DB + cross-file DB↔watchlist (site IDs/URLs, feed IDs/rss_urls); --verbose emits clickable file:line links
    ...
    standardize-feed-keys.js — reorder feed/site keys to canonical order (FEED/SITE_KEY_ORDER) + JSDoc; handles database + watchlist (reason), flags: --file, --dry-run, --check
